...
# === CONFIGURATION ===
OPENAI_API_KEY = (
    "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"  # <-- Your OpenAI API key here
)
...
def update_firestore(character_name, image_url):
    """Update Firestore document for the character with the new image URL."""
    print(f"Updating Firestore for character '{character_name}'...")
    db = firestore.client()
    doc_ref = db.collection("ai_characters").document(character_name)
    doc_ref.set({"profilePictureUrl": image_url}, merge=True)
    print(f"Firestore updated for '{character_name}'.")
