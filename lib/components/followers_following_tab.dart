...
class FollowersFollowingTab extends StatefulWidget {
  ...
  final String userId; // The ID of the user whose followers/following we're viewing
...
class _FollowersFollowingTabState extends State<FollowersFollowingTab> {
  ...
  void initState() {
    ...
    displayUserList = {
      "followers": List<Map<String, dynamic>>.from(widget.userList["followers"] ?? []),
      "following": List<Map<String, dynamic>>.from(widget.userList["following"] ?? [])
    };
    ...
    return Column(
      children: [
        ...
        Flexible(
            flex: 7,
            child: messageShown 
              ? Center(child: Text("No ${followersSelected ? "Followers" : "Following"} yet"))
              : buildUserList()),
        Flexible(
          flex: 1,
          child: Container(
...
class FollowButton extends StatefulWidget {
  ...
  const FollowButton({super.key, required this.otherUserId, required this.otherUserName });
      ...
      if (currentUserId != null) {
        ...
        if (userProfile != null && userProfile.containsKey('following')) {
          ...
          for (var followedUser in following) {
            ...
            } else if (followedUser is String && followedUser == widget.otherUserId) {
              isCurrentlyFollowing = true;
              break;
            }
          }
    ...
    return TextButton(
      style: ButtonStyle(
        backgroundColor: WidgetStateProperty.all<Color>(
          isFollowing
              ? Colors.blue
              : Colors.transparent, // Filled when following, transparent when not
        ),
