...
/// Base URL for public-read GCS bucket where Unity bundles are stored.
const _gcsBucketBase = 'https://storage.googleapis.com/inzone-unity-bundles/iOS';
...
class GameHubService {
  ...
  Future<List<UnityGame>> fetchGamesByCategory(String category) async {
    final snapshot = await _gamesRef.get();
    return snapshot.docs
        .map((doc) => UnityGame.fromFirestore(doc))
  ...
  Future<void> downloadGame(
    ...
    try {
      ...
      final url = '$_gcsBucketBase/${game.bundleFile}';
